...
## 当面板中的坐标按钮被切换时发出。
## occupy_map_cells_property_editor.gd 会监听这个信号，并把新数组写回 OccupyMap.cells。
signal cells_changed(new_cells: Array[Vector2i])
## 请求在独立弹窗中打开可视化面板。
signal popup_requested

## 坐标面板以边界 min~max 展示合法区域，默认左上角为 (-1, -1)。
## 默认空 cells 时显示 (-1,-1)~(2,2) 的 4×4 网格（含 (0,0) 一侧 1 格预留边距）。
const DEFAULT_VIEW_MIN_X := -1
const DEFAULT_VIEW_MAX_X := 2
const DEFAULT_VIEW_MIN_Y := -1
const DEFAULT_VIEW_MAX_Y := 2
const MIN_VIEW_SPAN := 1
const MAX_VIEW_SPAN := 25
...
## 当前可见编辑范围的边界。
var view_min_x := DEFAULT_VIEW_MIN_X
var view_max_x := DEFAULT_VIEW_MAX_X
var view_min_y := DEFAULT_VIEW_MIN_Y
var view_max_y := DEFAULT_VIEW_MAX_Y
...
## 绑定当前正在编辑的 OccupyMap 资源。
func set_occupy_map_resource(new_occupy_map_resource: OccupyMap) -> void:
...
func set_cells(new_cells: Array[Vector2i]) -> void:
	...
	var view_changed := _expand_view_to_fit_cells()
	if !is_node_ready():
		return
	if view_changed:
		_build_layout()
...
func _build_layout() -> void:
	...
	grid_container.columns = view_max_x - view_min_x + 1
	for cell_y in range(view_min_y, view_max_y + 1):
		for cell_x in range(view_min_x, view_max_x + 1):
...
## 调整可见坐标范围，不直接修改 OccupyMap.cells 数据。
## x+/y+ 向正方向扩展可见范围；x-/y- 从远端缩小可见范围。
func _change_view_bound(axis: Vector2i, amount: int) -> void:
	if axis == Vector2i.RIGHT:
		if amount > 0:
			view_max_x += 1
		elif view_max_x > view_min_x:
			view_max_x -= 1
	elif axis == Vector2i.DOWN:
		if amount > 0:
			view_max_y += 1
		elif view_max_y > view_min_y:
			view_max_y -= 1
...
func _refresh_buttons() -> void:
	if !is_node_ready() or grid_container == null:
		return
...
## 若 cells 超出当前 view，自动扩展边界（带 1 格 padding）。
## 不会在 cells 变少或为空时自动缩小视图。
func _expand_view_to_fit_cells() -> bool:
	if cells.is_empty():
		return false

	var new_min_x := view_min_x
	var new_max_x := view_max_x
	var new_min_y := view_min_y
	var new_max_y := view_max_y

	for cell in cells:
		if cell.x - 1 < new_min_x:
			new_min_x = cell.x - 1
		if cell.x + 1 > new_max_x:
			new_max_x = cell.x + 1
		if cell.y - 1 < new_min_y:
			new_min_y = cell.y - 1
		if cell.y + 1 > new_max_y:
			new_max_y = cell.y + 1

	var view_changed := (
		new_min_x != view_min_x
		or new_max_x != view_max_x
		or new_min_y != view_min_y
		or new_max_y != view_max_y
	)
	view_min_x = new_min_x
	view_max_x = new_max_x
	view_min_y = new_min_y
	view_max_y = new_max_y
	return view_changed
...
func _connect_resize_buttons() -> void:
	...
	x_decrease_button.pressed.connect(_change_view_bound.bind(Vector2i.RIGHT, -1))
	x_increase_button.pressed.connect(_change_view_bound.bind(Vector2i.RIGHT, 1))
	y_decrease_button.pressed.connect(_change_view_bound.bind(Vector2i.DOWN, -1))
	y_increase_button.pressed.connect(_change_view_bound.bind(Vector2i.DOWN, 1))
...
func _refresh_resize_buttons() -> void:
	...
	var view_width := view_max_x - view_min_x + 1
	var view_height := view_max_y - view_min_y + 1

	x_decrease_button.disabled = view_width <= MIN_VIEW_SPAN
	x_increase_button.disabled = view_width >= MAX_VIEW_SPAN
	y_decrease_button.disabled = view_height <= MIN_VIEW_SPAN
	y_increase_button.disabled = view_height >= MAX_VIEW_SPAN
